...
func TestBoltStorage_TodoOperation(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Cannot retriev files: %v", err)
	}

	retrieved.Title = "Update Title"
	err = storage.UpdateTodo(retrieved)
	if err != nil {
		t.Errorf("Cannot Update: %v", err)
	}
...
func TestBoltStorage_Sorting(t *testing.T) {
	...
	for _, todo := range todos {
		if err := storage.SaveTodo(todo); err != nil {
			t.Fatalf("Failed to save the todo: %v", err)
		}
	}
	...
	if !sorted[len(sorted)-1].Completed {
		t.Errorf("The last todo should be completed:")
	}
}
...
func TestBoltStorage_Streak(t *testing.T) {
	...
	storage, err := NewBoltStorage(dbPath)
	if err != nil {
		t.Fatalf("Unalble to open a storage: %v", err)
	}
	defer storage.Close()

	streak, err := storage.GetStreak()
	if err != nil {
		t.Errorf("Unable to fetch streak: %v", err)
	}
	...
	if err != nil {
		t.Errorf("Update streak failed: %v", err)
	}
...
func TestGetToUpcomingTodos(t *testing.T) {
	now := time.Now()
...
func TestGetTodosWithoutDeadline(t *testing.T) {
	...
	noDeadline := GetTodosWithoutDeadlines(todos)
